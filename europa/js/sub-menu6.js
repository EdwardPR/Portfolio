  ...
  onClick: function (event) {
  	...
  	switch (button) {
		...
		case 'resetMarksRim':
			...
			break;
		case 'downLinkTimesRim':
			var invisibleDownlinks = document.querySelectorAll('.downlinkTimes');
			for (var i = 0; i < invisibleDownlinks.length; i++) {
				if (!invisibleDownlinks[i].getAttribute('visible')) {
					invisibleDownlinks[i].setAttribute('visible', true);
				} else {
					invisibleDownlinks[i].setAttribute('visible', false);
				};
			};
			break;
		case 'transmitTimesRim':
			var invisibleUplinks = document.querySelectorAll('.uplinkTimes');
			for (var i = 0; i < invisibleUplinks.length; i++) {
				if (!invisibleUplinks[i].getAttribute('visible')) {
					invisibleUplinks[i].setAttribute('visible',true);
				} else {
					invisibleUplinks[i].setAttribute('visible', false);
				};
			};
			break;
  	};
...
AFRAME.registerComponent('cursor-controls', {
  init: function () {
 ...
 onEnterVR: function (event) {
    ...
    document.querySelector('a-cursor').components.cursor.pause();
  ...
  */
  onExitVR: function (event) {
    ...
    document.querySelector('a-cursor').components.cursor.play();
    // this.play();
  }
});
